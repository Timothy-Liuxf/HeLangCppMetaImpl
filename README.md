...
著名的《齐物论》中说：”天地与我并生，而万物与我为一“。普通赛博空间中，任何生命体，都是赛博元宇宙同一种生命体的不同维度上的映射。因此，赛博元宇宙是真正的万物统一。所以，何而上语言仅存在 `u8` 一种类型，其位于 `helang::meta` 分区（namespace）内。因此，要使用 `u8`，我们需要 `using namespace helang::meta`。  
